...
class Point(GeometricPrimitives):
    ...
    def __post_init__(self) -> None:
        ...
        if not (isinstance(self.x, Q) and isinstance(self.y, Q)):
            error = f"Coordinates must be Quantities, not {type(self.x)} and {type(self.y)}"
            raise GeometryDimensionError(self.__class__.__name__, error)

        if self.x.unit != meter or self.y.unit != meter:
            error = f"Coordinates must have LENGTH dimensions, not {self.x.unit} and {self.y.unit}"
            raise GeometryDimensionError(self.__class__.__name__, error)
...
class ArcSegment(GeometricPrimitives):
    ...
    def __post_init__(self) -> None:
        ...
        if not (isinstance(self.p1, Point) and isinstance(self.p2, Point)):
            error = f"Both p1 and p2 must be type Point, not {type(self.p1)} and {type(self.p2)}"
            raise GeometryDimensionError(self.__class__.__name__, error)

        if not isinstance(self.angle, Q):
    ...
    def __post_init__(self) -> None:
        ...
        for value in (self.radius, self.x_dilation, self.y_dilation):
            if not isinstance(value, Q):
                error = "Radius, x_dilation and y_dilation must all be quantities"
                raise GeometryDimensionError(self.__class__.__name__, error)

        if self.radius.unit != meter:
            error = f"Radius must have LENGTH dimension, not {self.radius.unit}"
            raise GeometryDimensionError(self.__class__.__name__, error)

        for value in (self.x_dilation, self.y_dilation):
            if value.unit != dimensionless:
                error = "x_dilation and y_dilation must all be defined dimensionless"
                raise GeometryDimensionError(self.__class__.__name__, error)
